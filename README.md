...
Static analysis in Python goes beyond linting and formatting by examining program semantics without executing it. While `Pylint` already performs some lightweight static checks, it does not provide full type validation or security scanning. To close these gaps, additional tools are recommended:

- **Mypy** (for Python type checking): Mypy uses Python’s type annotations (PEP 484) to validate function signatures, variable types, and interactions between modules. This helps catch subtle bugs, such as passing a string where an integer is expected, before they reach runtime.

- **Bandit** (for python-specific security analysis): Bandit focuses on identifying common security issues: use of eval, weak cryptographic functions, hardcoded secrets, or insecure subprocess usage. Together, they extend the safety net beyond style and conventions, ensuring code is not only readable but also robust and secure.
